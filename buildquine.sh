...
PARENTSHA=$(cat registry/docker/$MANIFEST2PARENT | jq -r '.config.digest' | sed 's/^sha256://')
...
cat registry/docker/$MANIFESTPARENT | jq -c '.config.container_config.image="sha256:'$PARENTSHA'" | .container_config.Cmd=["/bin/sh", "-c", "#(nop) ADD multi:foo in / "] | .container_config.Image="sha256:'$PARENTSHA'" | .history += [{"created": "2017-11-29T15:29:13.963036127Z","created_by":"/bin/sh -c #(nop) ADD multi:foo in / "}] | .rootfs.diff_ids += ["sha256:@DATASHA@"]' > rawdata/manifest1.json.tmpl

cat registry/docker/$MANIFEST2PARENT | jq '.layers+=[{"mediaType":"application/vnd.docker.image.rootfs.diff.tar.gzip","size":"@DIFFSIZE@","digest":"sha256:@DIFFSHA@"}] | .config.size="@MANIFESTSIZE@" | .config.digest = "sha256:@MANIFESTSUM@"' > rawdata/manifest2.json.tmpl
